...
abstract Class FormObject(){
	...
	/* Autor: Franchesco Fonseca, Webmized.com.
	 ...
	 * regla en mayuscula.
	 * 
	 * POR LO TANTO, si ud desea agregar una nueva regla siga el mismo patro para el nombre del método que la representa:
	 * protected function ruleMiregla($attribute){...}  donde a través del parametro attribute llegara el nombre del 
	 * atributo a validar.
	 * 
	 * POR ULTIMO, si el metodo que usted cree para representar una regla, cumple la condicion de error, añada el mensaje 
	 ...
	 * 
	 * donde en esa linea de codigo simplemente estamos cargando el mensaje de error en un array donde deben estar los errores,
	 * para luego mostrarlos al usuario en el formulario, el valor que se carga no es mas que el label definido para el atributo
	 * concatenado a un string complementario que permita entender la natturaleza el error.
	 *
	 *Información IMPORTANTE sobre los [FILTROS]:
	 * 
	 * Los filtros aplican la misma metodología antes descrita para las REGLAS, solo que no se manejan mensajes de error,
	 * sustituye un valor dado de un atributo por el valor con el filtro ya aplicado.
	 * 
	...
	*/
	...
	/*
	* las reglas a validar se almacenarán durante la línea de ejecución en este array vacío por defecto de la siguiente manera:
	...
	*/
	...
	/*
	* los filtros a aplicar se almacenarán durante la línea de ejecución en este array vacío por defecto de la siguiente manera:
	...
	*/
	...
	/*
	* los errores encontrados al validar las reglas se almacenarán en este array vacío de la siguiete manera para luego ser mostrados al usuario:
	* ['attribute1' => ['mensaje del error 1', 'mensaje del error N...'], 'attribute34' => ['mensaje del error 1', 'mensaje del error N...'],]
	*
	*/
	...
	protected function ruleDouble($value)
	{
		...
		if(!is_double($this->{$attribute})){
			...
			$this->addError($attribute, $this->attributeLabels($attribute)." "."debe ser un número de doble precisión");
		}
	...
	public function addFilter($filter)
	{
		...
		if(!is_array($filter)){
			...
			throw new Exception("Malformación de filtro de formulario, un filtro debe ser un array con la siguiente forma ['attribute' => attributeName, 'filter' => filterName]");	
		}

		if(!array_key_exists('attribute', $filter) || !array_key_exists('rule', $filter)){

			throw new Exception("Malformación de filtro de formulario, un filtro representado en un array debe tener obligatoriamente la llave attribute y la llave filter");
		}

		if(count($filter) < 2 || count($filter) > 2){
			
			throw new Exception("Malformación de filtro de formulario, un filtro puede tener solo 2 posiciones en el array");
		}
	...
	public function validate()
	{
		...
		foreach($this->$_formfilters as $filterToApply){
			...
			if(!method_exists($this, $methoTobeUsed)){
				...
				throw new Exception("El método {$methoTobeUsed} no existe, no se puede filtrar el atributo {$attributeToEval}");
			}
		...
		foreach($this->$_formRules as $ruleToEval){
			...
			if(!method_exists($this, $methoTobeUsed)){
				...
				throw new Exception("El método {$methoTobeUsed} no existe, no se puede validar el atributo {$attributeToEval}");
			}
